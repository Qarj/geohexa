...
def compress(num, cell_size):
    ...
    if (args.verbose):
        print ('base36 digit: ', base36, "(" +str(int(cell)) +")  cell_size_degrees[", cell_size, ']   fragment[', num, ']  remainder_to_go[', remainder, ']', sep='')
...
def latlon_to_geohexa(lat, lon, accuracy=3):
    ...
    while True:
        ...
        if (args.verbose):
            ...
            print('Current lat *lon* cell size meters: ', lat_cell_size,' , ', lon_cell_size, '   error in meters[', error, ']\n...',sep='')
...
parser.add_argument('--acc', dest='acc', required=False, action='store', help='Accuracy for conversion to geohexa in meters, default is 3', default=3, type=float)
parser.add_argument('--hexa', dest='hexa', required=False, action='store', help='geohexa', default='')
parser.add_argument('--version', action='version', version=version)
parser.add_argument('--verbose', action='store_true', dest='verbose', default=False, help='Will output extra info on calculation steps')
parser.add_argument('--null', action='store_true', dest='null', default=False, help='Will output the lat and lon of a null geohexa')
